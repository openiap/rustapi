...
impl Client {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            agent_version: Arc::new(std::sync::Mutex::new("0.0.12".to_string())),
            event_sender: ces,
            event_receiver: cer,
    ...
    pub async fn load_config(&self, strurl: &str, url: &url::Url) -> Option<Config> {
        ...
        if enable_analytics {
            ...
            match otel::init_telemetry(&agent_name, &agent_version, "0.0.12", strurl, otel_metric_url.as_str(), &self.stats) {
                Ok(_) => (),
                Err(e) => {
