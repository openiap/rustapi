...
(async () => {
    ...
    try {
        ...
        let eventid = client.on_client_event((event) => {
            client.info("client event", event);
        });
        ...
        if (signin_result.success) {
            ...
            if(test_sync) {
                for (y = 0; y < 1; y++) {
                    for (let i = 0; i < 15; i++) {
                        ...
                        client.info("query.sync.Got", query_result.length, "results");
                    }
                }
            }
            if(test_async) {
                ...
                for(y = 0; y < 1; y++) {
                    ...
                    client.info("query_async, wait for reply");
                    client.info(
            ...
            if(test_async) {
                ...
                client.info("aggregate_async success", aggregate_async_result.length, " results");

            }
            if(test_sync) {
                const aggregate_result = client.aggregate({ collectionname: "entities", aggregates: "[]", explain: false });
                client.info("aggregate success", aggregate_result.length, " results");
            }
            ...
            if(test_async) {
                ...
                client.info("download async success", download_async_result);
            ...
            if(test_watch) {
                ...
                const watch_result = client.watch({ collectionname: "entities", paths: "" }, (event, count) => {
                    client.info("watch " + event.operation + " #", count, " for " + event.document._type + " / " + event.document.test);
                    eventcount++;
                });
                client.info("watch created as", watch_result);
            ...
            if(test_async) {
                ...
                client.info("insert_one async success", insert_one_async_result._id);
            ...
            if(test_sync) {
                ...
                client.info("update_one success", update_one_result);
    
                const delete_one_result = client.delete_one({ collectionname: "entities", id: insert_one_result._id });
                client.info("delete_one success", delete_one_result == 1, delete_one_result);
            }
            ...
            if(test_async) {
                ...
                client.info("insert_or_update_one success", insert_or_update_one_async_result._id, insert_or_update_one_async_result.age);
                ...
                client.info("delete_many success", delete_many_query_async_result);
            }
            ...
            if(test_sync) {
                ...
                client.info("insert_or_update_one success", insert_or_update_one_result._id, insert_or_update_one_result.age);
                ...
                client.info("delete_many success", delete_many_query_result);
            }
            ...
            if(test_async) {
                ...
                client.info("push_workitem_async success", push_workitem_async_result);

                let pop_workitem_async_result = await client.pop_workitem_async({ wiq: "rustqueue" });
                client.info("pop_workitem_async success", pop_workitem_async_result);

                client.info("set pop_workitem_async_result.state = successful");
                ...
                client.info("update_workitem_async success", update_workitem_async_result);
                ...
                if(test_multiple_workitems) {
                    ...
                    client.info("push_workitem_async (multiple) success", (await Promise.all(promises)).map(result => result.id));

                    let workitem = await client.pop_workitem_async({ wiq: "rustqueue" });
                    do {
                        client.info("pop_workitem async (multiple) success", workitem);
                        workitem.state = "successful";
                        await client.update_workitem_async({ workitem });
                        client.info("update_workitem async (multiple) success");
            ...
            if(test_sync) {
                ...
                client.info("push_workitem success", push_workitem_result);

                client.info("now pop the workitem");

                let pop_workitem_result = client.pop_workitem({ wiq: "rustqueue" });
                client.info("pop_workitem success", pop_workitem_result);

                client.info("set pop_workitem_result.state = successful");
                pop_workitem_result.state = "successful";
                const update_workitem_result = client.update_workitem({ workitem: pop_workitem_result});
                client.info("update_workitem success", update_workitem_result);
                ...
                if(test_multiple_workitems) {
                    for(let i = 0; i < 10; i++) {
                        ...
                        client.info("push_workitem (multiple 2) success", push_workitem_result);
                    }
                    let workitem = client.pop_workitem({ wiq: "rustqueue" });
                    do {
                        client.info("pop_workitem success", workitem);
            ...
            if(test_message_queue) {
                ...
                const queuename = client.register_queue({ queuename: "testq" }, (event) => {
                    client.info("#", queuecount, "queue". event?.queuename, "event from", event?.replyto, ":", event?.data);
                    queuecount++;
                });
                client.info("queue registered with name", queuename);
                client.queue_message({ queuename, data: "{\"message\": \"test 1\"}" });
                client.queue_message({ queuename, data: "{\"message\": \"test 2\"}" });
                do {
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    // client.info("#", queuecount, "queue event loop");
                } while (queuecount < 2);
                client.info("queue event loop done");
                client.unregister_queue(queuename);
                client.info("Un registered queue", queuename);
            }
            if(test_exchange) {
                let exchangecount = 0;
                const queuename = client.register_exchange({ exchangename: "testexc" }, (event) => {
                    client.info("exchange event " + event.exchangename + " from " + event.replyto + " / " + event.data);
                    exchangecount++;
                });
                client.info("exchange registered with queue name", queuename);
                ...
                client.info("exchange event loop done");
                client.unregister_queue(queuename);
            }
        } else {
        ...
        if (error instanceof ClientError) {
            client.error(`An error occurred: ${error.message}`);
        } else {
            client.error(`An unexpected error occurred: ${error}`);
        }
    } finally {
        clearInterval(_int);
        if(client) {
