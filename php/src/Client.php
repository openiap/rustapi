...
class Client {
        ...
        while ($hadone) {
            ...
            do {
                ...
                try {
                    ...
                    try {
                        if(is_callable($callback)) {
                            $result = $callback($event, $event_counter);
                            if ($result !== null && $result != "" && $replyto != null && $replyto != "") {
                                // is $result a string ?
                                if (is_string($result)) {
                                    $me->queue_message($replyto, $result, ['striptoken' => true, 'correlation_id' => $correlation_id]);
                                } else {
                                    $me->queue_message($replyto, json_encode($result), ['striptoken' => true, 'correlation_id' => $correlation_id]);
                                }
                            }
                        }
                    } catch (Exception $error) {
    ...
    public function rpc($queuename, $payload, $options = array()) {
        ...
        if (!$response->success) {
            $error_message = FFI::string($response->error);
            $this->ffi->free_rpc_response($response);
            throw new Exception($error_message);
        }
